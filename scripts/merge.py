...
def merge(data_dir):
    ...
    post.merge_process_files(path.join(data_dir, "averaged"), cleanup=True)
    set_paths = glob(path.join(data_dir, "averaged", '*.h5'))
    post.merge_sets(path.join(data_dir, "averaged", "averaged_new.h5"), set_paths, cleanup=True)
    shutil.move(path.join(data_dir, 'averaged', 'averaged_new.h5'), path.join(data_dir, 'averaged', 'averaged.h5'))
